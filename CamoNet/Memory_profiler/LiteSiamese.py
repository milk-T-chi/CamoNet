...
class ProjectionPredictor(tf.keras.Model):
    def __init__(self,input_size,**kwargs):
        super(ProjectionPredictor, self).__init__(**kwargs)
        ...
        self.projection_head_predictor = tf.keras.Sequential(layer_list)
    

    def call(self, inputs, training=True, **kwargs):
        return self.projection_head_predictor(inputs, training=training)
    
    def get_sub_layers(self):
        layers = self.projection_head_predictor.layers
        for layer in layers:
...
class LiteSiameseModel(tf.keras.Model):
    ...
    def train_step(self, unlabeled_images):
        augmented_images_1 = unlabeled_images
        augmented_images_2 = unlabeled_images
