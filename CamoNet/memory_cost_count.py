...
alpha_list = [1.4, 1.3, 1.0, 0.75, 0.5, 0.35]
# lite_residual_list = []
image_channels = 3
batch_size = 1
...
for i in range(len(alpha_list)):
    ...
    for image_size in image_size_list:
        ...
        for k in tqdm(range(len(total_config_t_n_now))):
            ...
            backbone_encoder_size, backbone_encoder_activation_size = profile_memory_cost(backbone_encoder,
                                                                                          batch_size=1)
            # print(backbone_encoder_size, backbone_encoder_activation_size)
            all_encoder_param_size = backbone_encoder_activation_size['param_size']
            all_encoder_act_size = backbone_encoder_activation_size['act_size'] * 2
            ...
            memory_cost_list.append(all_encoder_memory_size)
            param_size_list.append(all_encoder_param_size)
            activation_size_list.append(all_encoder_act_size)
            flops_list.append(flops)
            # print("计算一个模型的时间",time.time() - start_time)
        model_config_now_df = pd.DataFrame(
            {'t_n': total_config_t_n_now, 'memory_cost_MB': memory_cost_list, 'param_size_MB': param_size_list,
             'activation_size_MB': activation_size_list, 'FLOPs': flops_list})
